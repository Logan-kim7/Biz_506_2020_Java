...
public class StudentServiceImplV1 implements StudentService{
	...
	// 필드 변수를 선언할때 접근제한자(private, public, protected)중에 한가지로
	// 설정할수 있다. 
	...
	// 접근을 허용하는 것이다.
	// 혹시 필드변수를 외부에서 접근할 필요가 있는 경우에는 getter, setter를 만든다.
	
	// 필드변수를 protected로 선언하기
	// protected는 private 접근제한자와 거의 유사한 형태이다.
	// 만약 현재 클래스를 다른 클래스를 만들떄 상속(extends)하여 사용하려고 할때
	// 상속받은 클래스에서 현재 클래스의 필드변수를 같이 상속받아서 사용하고자 할때는
	// 변수를 protected로 선언한다.
	
	// 현재 클래스 에서는 private처럼 작동하고
	...
	public boolean inputStudent() {
		...
		System.out.println("학번을 입력해주세요 입력종료>>[END] :  ");
		...
		// 유효성검사는 개발자 입장에서는 매우 귀찮고 단순노동일 경우가 많지만
		// 사용자입장에서는 유효성검사가 많으면 그만큼 잘못된 데이터를 입력하는 경우를
		// 방지할수 있다.
		try {
			intGrade = Integer.valueOf(strGrade);
		}catch (Exception e) {
			System.out.println("학년은 숫자만가능!!!");
			System.out.println(std.getNum() + "학번 부터 다시입력하세요");
			return true;
		}
		if(intGrade < 1 || intGrade >4) {
			System.out.println("학년은 1 ~ 4 학년 까지만 가능");
			System.out.println(std.getNum()+"학번 부터 다시입력해주세요");
			return true;
		}
		std.setGrade(intGrade);
		
		System.out.println("전공을 입력하세요>>:  ");
		String strDept = scn.nextLine();
		std.setDept(strDept);
		//모든 입력이 정상처리 되었으면
		// List에 학생정보를 추가한다.
		...
		// 제일 끝의 return문은 한 한생의 정보가 모두 정상 입력이 되면
	...
	public void studentList() {
		...
		System.out.print("학번\t 이름 \t학년 \t전공\n");
		int listsize = studentList.size();
		for(int i = 0 ; i < listsize; i ++) {
			System.out.printf("%s\t%s\t%d\t%s\n", studentList.get(i).getNum(),studentList.get(i).getGrade(),studentList.get(i).getName(),
					studentList.get(i).getDept());
