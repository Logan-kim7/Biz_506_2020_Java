
public class Hello_03 {

	// args : arguments라고 읽는다.
	// 매개변수, parameter
	
	public static void main(String[] args) {
		System.out.println("=================");
		System.out.println("16단 구구단");
		// 16 x 1 이라고    consol에 보여라
		System.out.println("-----------------");
		System.out.println(" 16 x 1 = " + 16 * 1);
		System.out.println(" 16 x 2 = " + 16 * 2);
		System.out.println(" 16 x 3 = " + 16 * 3);
		System.out.println(" 16 x 4 = " + 16 * 4);
		System.out.println(" 16 x 5 = " + 16 * 5);
		System.out.println(" 16 x 6 = " + 16 * 6);
		System.out.println(" 16 x 7 = " + 16 * 7);
		System.out.println(" 16 x 8 = " + 16 * 8);
		System.out.println(" 16 x 9 = " + 16 * 9);
		System.out.println("=================");
	}

}
