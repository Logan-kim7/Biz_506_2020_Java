 ...
 * 클래스에 진입점 method를 생성하여 코드를 실행해볼수 있게 된다.
 * 클래스만 작성된 상태에서 진입 method가 없으면 스스로 아무런 코드도 실해애할 수 없다.
 * 
 * 
 */
public class MemberEx {
	 ...
	 * 제임먼저 호출하는 method이고 그 문법이 정해져 있다.
	 * 
	 * agrs 배열은 현재 클래스를  cmd 창에서 실행할때
	...
	public static void main(String[] args) {
		...
		/*
		 * 변수를 선언만 한 상태에서 변수를 읽으려고 시도를 하면 
		 * 문법 오류가 나타난다.
		 * 선언만 한  상태의 변수는 어떤 겂이 저장되어 있는지 알수 없는 상태이다.
		 * 이 상태에서 값을 읽는 동작을  수행하게 되면 
		 * 시스템 메모리에 저장된 값이 노출되는 현상이 발생을 한다.
		 * 
		 * 그래서 자바에서 메모리에 초기화 되지 않은 변수에 담긴 값은 
		 * 확인할수 없도록 언어 문법구조가 형성되어  있다.
			int num3;
		 * 
		 */
		...
		/*
		 * 변수를 선언하고 초기화를 할때
		 * 숫자형(정수, 실수..) 변수는 최소한 0 이라도 저장을 해주는 것이 좋다.
